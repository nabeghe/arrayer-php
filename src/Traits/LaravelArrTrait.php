...
namespace Nabeghe\Arrayer\Traits;
...
trait LaravelArrTrait
{
    ...
    {
        if (is_null($callback)) {
            return empty($array) ? Cally::value($default) : end($array);
        }
    ...
    public static function get($array, $key, $default = null)
    {
        ...
        if (!Str::contains($key, '.')) {
            return $array[$key] ?? Cally::value($default);
        }
...
//    }
     ...
     */
    public static function pregReplace($pattern, array $replacements, $subject)
    {
    ...
    public static function random($array, $number = null, $preserveKeys = false)
    {
        ...
        if (PHP_VERSION_ID >= 80200) {
            ...
            if (!is_array($array) || !is_int($requested) || $requested <= 0) {
                $keys = [];
            } else {
                $keys = array_keys($array);
                shuffle($keys);
                $keys = array_slice($keys, 0, $requested);
            }
        }
